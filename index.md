...
<h2>Explanations Ontology</h2>
  <p>We have designed an explanations ontology that captures all the aspects of explanations related to their generation from a system's perspective and accounting for factors from an end-user's perspective. Through modeling the components involved in the process of explanation generation, we provide a means for system designers to translate their user requirements gathered from user studies to explanations that can be generated by their systems. Through our ontology schema, we show how we are able to support the modeling of different literature-derived explanation types that help address the diverse needs of user-centric explainability.</p>
